export const ROOMS = [
    {
        id: 1,
        roomName: "Empire Room",
        roomType: "rehersal",
        roomPrice: "$25/hr",
        roomImage: "/assets/images/brett-jordan-WzzGU9dwMnU-unsplash.jpg",
        roomDescription: "Our largest room at 300 square feet. It is a professionally designed acoustic space and sounds great. Fits 8.",
        equipment: [
            "Marshall Guitar Cabinet Amp w/ external Head Amp", 
            "Fender Princeton Guitar Amp",
            "Line 6 Guitar Combo Amp",
            "Yamaha Portable PA w/ 2 Yamaha Speakers",
            "Full Pearl Drum Kit w/ Stool &amp; Cymbals",
            "Am peg Bass Amp",
            "3 Microphones",
            "4 Cords",
            "3 Stands",
            "Korg X50 Professional 61 Key Syth/Keyboard"
        ]
    },
    {
        id: 2,
        roomName: "Corner Room",
        roomType: "rehersal",
        roomPrice: "$25/hr",
        roomImage: "/assets/images/jonathan-velasquez-c1ZN57GfDB0-unsplash.jpg",
        roomDescription: "A great sounding 250 square foot space with professionally designed acoustic treatments. Fits 6 comfortably.",
        equipment: [
            "Marshall Guitar Cabinet Amp w/ external Head Amp", 
            "Fender Princeton Guitar Amp",
            "Yamaha Portable PA w/ 2 Yamaha Speakers",
            "Full Pearl Drum Kit w/ Stool &amp; Cymbals",
            "Am peg Bass Amp",
            "2 Microphones",
            "2 Cords",
            "2 Stands",
            "Korg X50 Professional 61 Key Syth/Keyboard"
        ]
    },
    {
        id: 3,
        roomName: "Recording Studio",
        roomType: "recording Studio",
        roomPrice: "$50/hr",
        roomImage: "/assets/images/marc-fanelli-isla-xo4ValczbuA-unsplash.jpg",
        roomDescription: "Our professional studio features the most up to date technology giving you access to the best quality.",
        equipment: [
            "Marshall Guitar Cabinet Amp w/ external Head Amp", 
            "Fender Princeton Guitar Amp",
            "Yamaha Portable PA w/ 2 Yamaha Speakers",
            "Full Pearl Drum Kit w/ Stool &amp; Cymbals",
            "Am peg Bass Amp",
            "2 Microphones",
            "2 Cords",
            "2 Stands",
            "Korg X50 Professional 61 Key Syth/Keyboard"
        ]
    },
]